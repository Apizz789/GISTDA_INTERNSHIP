...
def MapView(request):
    ...
    if request.method == "POST":
        ...
        #         form_input.save()
        #         dataform['name'] = form_input.cleaned_data.get('siteBox')
        #         dataform['coordinate'] = form_input.cleaned_data.get('inputBox')
        #         dataform['status'] = 'ok'
                
        #         return JsonResponse(dataform)
...
def CesiumView(request):
    return render(request,"cesium.html")

def MapPluemView(request):
    return render(request,"mapPluem.html")